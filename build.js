...
for(const t of [tags.js,tags.wizardJs,tags.materialsJs,tags.jobsJs,tags.authJs,tags.localesJs,tags.i18nJs,tags.i18nBehaviorJs,tags.pwaJs]){const src=t.match(/src="([^"]+)/)?.[1];if(src&&!html.includes(src))html=html.replace('</body>',`  ${t}\n</body>`)}
html=html
 ...
 .replace(/currentLanguage==='ar'\?'ar':'he-IL'/g,"currentLanguage==='ar'?'ar':currentLanguage==='en'?'en-US':'he-IL'")
 .replace("function fmt(n){ return (Math.round(n*100)/100).toLocaleString('he-IL'); }","function fmt(n){ const l=currentLanguage==='ar'?'ar':currentLanguage==='en'?'en-US':'he-IL'; return (Math.round(n*100)/100).toLocaleString(l); }")
...
for(const file of ['stitch-hybrid.css','stitch-hybrid.js','wizard-v2.css','wizard-v2.js','materials-v2.js','jobs-v2.js','auth-v2.js','i18n.css','i18n-locales.js','i18n-admin-locales.js','i18n.js','i18n-behavior.js','manifest.webmanifest','pwa-icon.svg','sw.js','pwa.js'])fs.copyFileSync(file,path.join(outDir,file));
fs.cpSync('vendor',path.join(outDir,'vendor'),{recursive:true});
console.log('Product Architecture V2 + full HE/AR/EN localization + language-aware AI/WhatsApp + admin localization + wizard + jobs + password reset + materials + PWA built to /dist');
