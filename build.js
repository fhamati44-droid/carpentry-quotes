...
for (const file of ['admin.html','stitch-hybrid.css','stitch-hybrid.js','wizard-v2.css','wizard-v2.js','manifest.webmanifest','pwa-icon.svg','sw.js','pwa.js']) {
  fs.copyFileSync(file, path.join(outDir, file));
}
fs.cpSync('vendor', path.join(outDir, 'vendor'), { recursive: true });

console.log('Product Architecture V2 + quote wizard + PWA built to /dist');
