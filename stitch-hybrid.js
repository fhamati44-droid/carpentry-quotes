...
(function(){
  ...
  function emphasizeMaterialPricing(){const settingsTab=$('#tab-settings');if(!settingsTab)return;const labels=$$('label',settingsTab);const label=labels.find(l=>/סוגי חומרים.*מחיר למ״ר/.test(l.textContent||''));if(!label||label.dataset.hyPriceReady)return;label.dataset.hyPriceReady='1';label.id='materialPricingSettings';label.style.fontSize='16px';label.style.color='#1e2229';const note=document.createElement('div');note.className='hint';note.textContent='הגדר כאן את המחיר שלך לכל חומר לפי מ״ר. המחיר משמש אוטומטית בכל הצעה.';label.insertAdjacentElement('afterend',note)}
  ...
  function run(){if(busy)return;busy=true;try{normalizeNavigation();addDashboard();hideLegacyInternalCosting();polishAiCopy();addWorkspaceIntro();emphasizeMaterialPricing();simplifyChrome();moveCustomerFirst();collapseAiOnMobile();enhanceActions();mobileFocus()}finally{busy=false}}
