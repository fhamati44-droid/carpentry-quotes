...
(function(){
 ...
 function translateString(value,l=lang){if(!value||l==='he')return value;let out=value;for(const [he,tr] of Object.entries(dict(l)).sort((a,b)=>b[0].length-a[0].length))if(out.includes(he))out=out.split(he).join(tr);return out}
 function translateNode(node){
   if(!node)return;
   if(node.nodeType===Node.TEXT_NODE){if(node.parentElement?.closest('script,style,[data-i18n-ignore]'))return;if(!originalText.has(node))originalText.set(node,node.nodeValue);node.nodeValue=translateString(originalText.get(node));return}
   ...
   [node,...qa('*',node)].forEach(el=>{if(el.closest?.('[data-i18n-ignore]'))return;const saved=originalAttrs.get(el)||{};['placeholder','title','aria-label','value'].forEach(a=>{if(a==='value'&&!['BUTTON','INPUT'].includes(el.tagName))return;if(el.hasAttribute?.(a)&&!(a in saved))saved[a]=el.getAttribute(a);if(a in saved)el.setAttribute(a,translateString(saved[a]))});originalAttrs.set(el,saved)});
 }
 function addSelector(){if(q('#cqLanguageSelect'))return;const sel=document.createElement('select');sel.id='cqLanguageSelect';sel.className='cq-language-select no-print';sel.setAttribute('aria-label','בחירת שפה');sel.innerHTML='<option value="he">עברית</option><option value="ar">العربية</option><option value="en">English</option>';sel.value=lang;sel.addEventListener('change',()=>setLanguage(sel.value));const header=q('header.topbar .row')||q('header.topbar');header?.insertBefore(sel,q('#langToggle')||q('#accountActions')||null);const auth=q('.auth-card');if(auth&&!q('#cqAuthLanguageSelect')){const clone=sel.cloneNode(true);clone.id='cqAuthLanguageSelect';clone.value=lang;clone.addEventListener('change',()=>setLanguage(clone.value));auth.insertBefore(clone,auth.firstChild)}qa('.lang-toggle').forEach(x=>x.style.display='none')}
 ...
 function syncLegacyLanguage(){try{currentLanguage=lang}catch(e){window.currentLanguage=lang}}
